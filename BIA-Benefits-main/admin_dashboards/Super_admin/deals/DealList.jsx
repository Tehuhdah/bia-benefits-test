...
function DealList() {
  ...
  const addDeals = async (values) => {
    ...
    try {
      const addressData = {
        province: Array.isArray(values.province)
          ? values.province.join(", ")
          : values.province,
        city: values.city,
              ...
              <table className="w-full p-2 rounded-lg ">
                <thead className="bg-gray-200">
                  <tr>
                    ...
                    <th className="p-2 w-1/4 text-left pl-5">
                      Corporate Sponsor
                    </th>
                  ...
                  {currentItems.map((deal, index) => (
                    <tr key={deal.id} className="hover:bg-gray-100">
                      ...
                      <td className="p-4 w-1/4">
                        {capitalizeFirstLetter(deal.corporatePartner)}
                      </td>
                      <td className="p-4 w-1/4">
                        {capitalizeFirstLetter(deal.addresses[0]?.province)}
                      </td>
                      <td className="p-4 w-1/4">
                        {capitalizeFirstLetter(deal.addresses[0]?.city)}
                      </td>
                    </tr>
                  ))}
                </tbody>
