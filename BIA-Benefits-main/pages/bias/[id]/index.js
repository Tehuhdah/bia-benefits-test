...
import Modal from "@/admin_dashboards/Modal";
...
const BiaDetails = () => {
  ...
  const currentBusinesses = bia?.businesses.slice(
    businessStartIndex,
    businessEndIndex
  );
      ...
      <ProtectedPage allowedRoles={["SUPER_ADMIN"]}>
        ...
        <div className="overflow-x-auto">
            ...
            <div className="border-t border-gray-200 px-4 py-5 sm:p-0">
              <dl className="sm:divide-y sm:divide-gray-200">
                ...
                <div className="py-3 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                  ...
                  <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                    {bia?.addresses[0]?.postalCode
                      ? bia?.addresses[0]?.postalCode.toUpperCase()
                      : "N/A"}
                  </dd>
                </div>
              </dl>
          ...
          <div className="bg-white  shadow rounded-lg border mt-4 p-6">
            ...
            <div className="px-4 py-5 sm:px-6">
              <h3 className="text-lg leading-6 font-medium text-gray-900">
                Businesses under {bia?.nameOfBia}
              </h3>
            </div>
            ...
            <div className="border-t border-gray-200 px-4 py-5 sm:p-0">
                    ...
                    <Link key={business.id} href={`/business/${business.id}`}>
                      <div className="overflow-x-hidden">
                        <table className="w-full p-2 rounded-lg overflow-hidden">
                          <tbody>
                            <tr key={business.id}>
                              <td className="p-4 text-left w-1/5 overflow-hidden">
                                {business.name}
                              </td>
                              <td className="p-4 text-left w-1/5 overflow-hidden">
                                {business.addresses[0]?.city}
                              </td>
                              <td className="p-4 text-left w-1/4 overflow-hidden">
                                {business.category}
                              </td>
                              <td className="p-4 text-left w-1/5 overflow-hidden">
              ...
              <PaginationLayout
                ...
                totalPages={Math.ceil(
                  (bia?.businesses.length || 0) / businessItemsPerPage
                )}
...
export const getServerSideProps = async ({ params }) => {
  try {
    const response = await fetch(`/api/bias/${params.id}`);
    if (response.ok) {
      const bia = await response.json();
      return {
        props: { bia },
      };
    } else {
